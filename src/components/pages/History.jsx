  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="relative mb-6">
        <div className="flex items-center gap-1 bg-surface-inset rounded-xl p-1 w-fit">
          {/* Sliding indicator */}
          <div
            className="absolute top-1 h-[calc(100%-8px)] bg-surface rounded-lg shadow-sm transition-all duration-300 ease-out"
          ...
          {TABS.map(({ key, label }) => (
            ...
            >
              ...
              {activeTab !== 'all' && key !== 'all' && key === activeTab && (
                <span className="ml-1.5 text-[10px] font-bold text-accent bg-accent-subtle px-1.5 py-0.5 rounded-full">
                  {filteredJobs.length}
                </span>
              )}
            </button>
      ...
      <div className="space-y-2" key={activeTab}>
        {filteredJobs.length === 0 ? (
          <div className="text-center py-16 animate-fade-in">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-surface-inset flex items-center justify-center">
            ...
            <p className="text-sm font-semibold text-text-primary mb-1">No {activeTab !== 'all' ? activeTab : ''} requests yet</p>
            <p className="text-xs text-text-secondary">New verification requests will appear here</p>
          </div>
        ) : (
